...
module Channels
  ...
  describe Channels::Podcasts::Collector do
    ...
    describe '.new' do
      ...
      it "queues requests for the collection of Podcasts" do
        q = Podcasts::Collector.new([@podcast]).queue
      ...
      it "requires an array of @podcasts" do
        lambda { Podcasts::Collector.new(@podcast)         }.should raise_error(ArgumentError)
        lambda { Podcasts::Collector.new(['not podcast'])  }.should raise_error(ArgumentError)
        lambda { Podcasts::Collector.new([@podcast])       }.should_not raise_error
      end
    end
    ...
    describe '#run!' do
      ...
      it "calls &on_complete for each @podcast" do
        ...
        collector = Podcasts::Collector.new(@podcasts) {|podcast_obj, podcast_xml| podcast_obj.foo(podcast_xml) }
        collector.run!
      end
    end
